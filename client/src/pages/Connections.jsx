  ...
  return (
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8'>
          <h1 className='text-3xl font-bold text-[#074240] mb-2'>Connections</h1>
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab)=>(
              <button onClick={() => setCurrentTab(tab.label)} key={tab.label} className={`
              ...
              <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
                ...
                <div className='flex-1'>
                  ...
                  <div className='flex max-sm:flex-col gap-2 mt-4'>
                    {
                      <button
                        onClick={()=>navigate(`/community/profile/${user._id}`)}
                        className='w-full p-2 text-sm rounded bg-gradient-to-r from-[#074240] to-[#12A8A3] active:scale-95 transition text-white cursor-pointer'
                      >
                        View Profile
                      </button>
                    }
...

                    {
                      currentTab === 'Connections' && (
                        <button
                          onClick={()=>navigate(`/community/messages/${user._id}`)}
